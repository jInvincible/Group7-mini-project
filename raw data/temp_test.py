...
for url_main in urls:
    ...
    for rounds in all_round_list:
        ...
        for match_info in all_matches_info:
            match_stage = rounds
            # get match_id, for missing matchid, receive '0'
            ...
            if match_id in ['1689', '2350', '2454', '2352', '2431', '2220', '2196', '2252']:
            ...
            if tag_starting_table.name not in ['h3', 'h2', 'link', 'style']:
                ...
                if match_id in ['2350', '2352']:
                    tag_h = tag_starting_table.tr.tr.findChildren("td", recursive=False)[0]
                else:
                    tag_h = tag_starting_table.tr.findChildren("td", recursive=False)[0]

                # get tag of away team starting
                if match_id in ['2350', '2352']:
                    tag_a = tag_starting_table.tr.tr.findChildren("td", recursive=False)[-1]
                else:
...
# for i, fullname in enumerate(df_starting['HT Fullname']):
#     for x, name in enumerate(df_starting['HT star name']):
#         if name in fullname:
#             df_starting['match_hgevent'][i] = df_starting['HT Goal'][x]
#
# fullname = df_starting['HT Fullname'][10]
# fullname = 'Nguyen Van Cea'
# name = df_starting['HT star name'][1]
# name.strip() in fullname.strip()
# print(name+'     '+fullname)
            # reset url_main to reuse in loops
        url_main = url_temp1
df_total_starting.to_excel('Fifa_world_cup_.xlsx', sheet_name='Match Details')
