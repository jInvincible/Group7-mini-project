...
for url_main in urls:
    ...
    for rounds in all_round_list:
        ...
        # for testing-only, initialize basic info (url_main, r_main, soup_main, all matches_info
        url_main = 'https://en.wikipedia.org/wiki/2018_FIFA_World_Cup_Group_G'
        r_main = requests.get(url_main)
        soup_main = bs4.BeautifulSoup(replace_tags_with_labels(r_main,
                                                               replacements=REPLACEMENTS).replace(
            '\n', ''), 'html.parser')
        soup_main = bs4.BeautifulSoup(soup_main.encode('UTF-8'),
                                      'html.parser')
        all_matches_info = [soup_main.findAll(attrs='footballbox')[3]]


        # collect needed data for each match of a round (group_stages and knockout stage define above)
